...
export class MatierPremier extends Component {
  ...
  createClick() {
    ...
    axios.post('https://localhost:7230/api/MatierPremier', {
  ...
  updateClick() {
    ...
    axios.put(`https://localhost:7230/api/MatierPremier/${this.state.id}`, {
      id: this.state.id,
      categoryName: this.capitalizeFirstLetter(this.state.DepartmentName),
      matiereType: this.capitalizeFirstLetter(this.state.matiereType)
    })
  ...
  deleteClick(id) {
    if (window.confirm('Are you sure?')) {
      axios.delete(`https://localhost:7230/api/MatierPremier/${id}`)
        .then(response => {
          if (response.status === 200) {
