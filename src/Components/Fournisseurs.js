...
export class Fournisseurs extends Component {
  ...
  createClick() {
    ...
    axios.post('https://localhost:7230/api/Fournisseurs', {
  ...
  updateClick() {
    ...
    axios.put(`https://localhost:7230/api/Fournisseurs/${this.state.idFournisseur}`, {
  ...
  deleteClick(idFournisseur) {
    if (window.confirm('Êtes-vous sûr?')) {
      axios.delete(`https://localhost:7230/api/Fournisseurs/${idFournisseur}`)
        .then(response => {
          if (response.status === 200 || response.status === 204) {
