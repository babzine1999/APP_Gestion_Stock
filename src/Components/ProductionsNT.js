...
export class ProductionsNT extends Component {
  ...
  refreshList() {
    axios.get('https://localhost:7230/api/ProductionNT') 
      .then(response => {
  ...
  createClick() {
    ...
    axios.post('https://localhost:7230/api/ProductionNT', {
  ...
  updateClick() {
    ...
    axios.put(`https://localhost:7230/api/ProductionNT/${this.state.idProduction}`, {
  ...
  deleteClick(id) {
    if (window.confirm('Are you sure?')) {
      axios.delete(`https://localhost:7230/api/ProductionNT/${id}`)
        .then(response => {
          if (response.status === 200 || response.status === 204) {
